...
public:
	ClassLuaFunction(lua_State* L,  C* c, const std::string& objName, const std::string& name, Ret(*function)(Args...)) : ClassLuaFunction(L, c, objName, name, FunctionType(function)) {};

	ClassLuaFunction(lua_State* L, C* c, const std::string& objName, const std::string& funcName, FunctionType function) : m_function(function), m_funcName(funcName), m_objName(objName), m_obj(c)
	{
		m_luaState = L;
		// Add pointer to this object to the closure
		lua_pushlightuserdata(m_luaState, (void*) static_cast<ILuaFunction*>(this));
...
private:
	...
	C*_get(lua_State *state)
	{
        /*LuaCallDispatcher::GetInstance().stackDump(state);
        std::cout <<"top: " << lua_tointeger(state, -2)<< std::endl;
		C *ret = (C *)lua_touserdata(state, -2);
...
public:
    ClassLuaFunction(lua_State* L, C* c, const std::string& objName, const std::string& name, void(*function)(Args...)) : ClassLuaFunction(L, c, objName, name, FunctionType(function)) {};
	...
	~ClassLuaFunction()
	{
		if (m_luaState != nullptr)
		{
            ...
            LuaCallDispatcher::GetInstance().stackDump(m_luaState);
            lua_pushnil(m_luaState);
            lua_setfield(m_luaState, -2, <#const char *k#>)*/
		}
	}

	int Apply(lua_State* L)
	{
        //LuaCallDispatcher::GetInstance().stackDump(L);
		std::tuple<C*> t = std::make_tuple(_get(L));
       // LuaCallDispatcher::GetInstance().stackDump(L);
		std::tuple<Args...> args = detail::GetArguements<Args...>(L);
        //LuaCallDispatcher::GetInstance().stackDump(L);
