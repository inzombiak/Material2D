...
{
public:
	LuaEnvironment(std::string name, lua_State* state)
	{
	...
	void RegisterMemberFunction(lua_State* L, Class *pClass, std::string metatableName, std::string name, Ret(Class::*function)(Args...), std::string funcName)
	{
        ...
        if(!lua_isnil(L, - 1))
        {
            ...
            m_objectMap[name]->AddFunction(classFunc, std::move(funcName));
            (pClass->*function)(1);
            return;
        }
	...
	void RegisterMemberFunction(lua_State* L, Class *pClass, std::string metatableName, std::string name, void(Class::*function)(Args...), std::string funcName)
	{
        ...
        if(!lua_isnil(L, - 1))
        {
            ...
            ClassLuaFunction<1, Class, void, Args...>*classFunc = new ClassLuaFunction<1, Class, Ret, Args...>(L, pClass, name, funcName, lambda);
            m_objectMap[name]->AddFunction(classFunc, std::move(funcName));
            return;
        }
...
private:
    ...
    inline void Clear()
    {
        lua_getfield(m_luaState, LUA_REGISTRYINDEX, m_environmentName.c_str());
        m_objectMap.clear();
        m_functionMap.clear();
    }
