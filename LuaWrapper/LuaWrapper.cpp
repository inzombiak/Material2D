...
void LuaWrapper::LoadScript(const std::string& environmentName, const std::string& filename)
{
	...
	m_environmentMap[environmentName] = std::unique_ptr<LuaEnvironment>(new LuaEnvironment(environmentName, m_luaState));
}
...
void LuaWrapper::CreateLuaEnvironment(const std::string& name)
{
	...
	m_environmentMap[name] = std::unique_ptr<LuaEnvironment>(new LuaEnvironment(name, m_luaState));
}
